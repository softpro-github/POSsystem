<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-ink leading-tight">Edit Category</h2>
    </x-slot>
                    ...
                    <div>
                        ...
                        <select id="tax_group_id" name="tax_group_id" class="mt-1 block w-full bg-surface-hover border-border-strong text-ink rounded-md shadow-sm">
                            ...
                            @foreach ($taxGroups as $group)
                                <option value="{{ $group->id }}" @selected(old('tax_group_id', $category->tax_group_id) == $group->id)>{{ $group->name }} ({{ $group->totalRate() }}%)</option>
                            @endforeach
                        </select>
